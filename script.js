...
enterBtn.addEventListener("click", () => {
  ...
  tryStartMusic(); // autoplay attempt after a user gesture
});
...
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape"){
    if (!zoomModal.classList.contains("is-hidden")) closeZoom();
    else if (!gallery.classList.contains("is-hidden")) hideGallery();
  }
});
...
window.onYouTubeIframeAPIReady = function () {
  ytPlayer = new YT.Player("yt", {
    ...
    playerVars: {
      autoplay: 0,
      controls: 1,          // keep YouTube controls available too
      rel: 0,
      modestbranding: 1,
      playsinline: 1
    },
...
function tryStartMusic(){
  if (!ytReady || !ytPlayer) return;
  // Attempt to autoplay after Enter click
  ytPlayer.unMute();
  isMuted = false;
  setMuteIcon();
  ytPlayer.setVolume(parseInt(vol.value, 10));
